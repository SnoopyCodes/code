...
int main() {
    ...
    for (int i = 0; i < N; i++) { next_shake[i] = i; cout << happy[i] << "\n";}
    sumshakes[0] = happy[0];
    ...
    while (M != 0 && !pq.empty()) {
        ...
        if (happy[ns] < min) {  //if there is still fight left put it back in there for later
            ...
            if (M <= 2 * (next_shake[index]+1)) {
                int rem = M % 2;
                int uses = M / 2;
                cout << next_shake[index] << " " << uses << "\n";
                cout << happiness << "\n";
                happiness += 2 * (sumshakes[next_shake[index]] -
                sumshakes[next_shake[index]-uses] + uses * happy[index]);
                cout << happiness << "\n";
                if (rem) {
                    happiness += happy[next_shake[index]-uses] + happy[index];
                }
